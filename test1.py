...
for p in my_open('model/user_factors_%f_0.000000.txt' % regular):
    user = p[0]
    user_pref[user] = np.array(map(float,p[1:]))

for p in my_open('model/poi_factors_%f_0.000000.txt' % regular):
    poi = p[0]
...
for target_user in testing:
    ...
    for p in hometown_poi[ot]:
        score[p] = user_pref[target_user].dot(poi_pref[p])
